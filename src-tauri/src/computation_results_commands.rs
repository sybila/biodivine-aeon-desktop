...
use crate::common::{ErrorMessage, JsonString};
...
fn get_witness_attractors(f_colors: &GraphColors, window_session_key: &str) -> Result<Value, ErrorMessage> {
    {
...
#[tauri::command]
pub fn get_bifurcation_tree(window_session_key: &str) -> Result<String, ErrorMessage> {
    let locked_tree = get_locked_tree(window_session_key);
    let read_tree = locked_tree.read().unwrap();
    if let Some(tree) = read_tree.as_ref() {
