...
pub fn menu_init() -> Menu {
    ...
    let budding_yeast_orlando = CustomMenuItem::new("budding_yeast_orlando", "Orlando - Budding Yeast");
    ...
    let examples_submenu = Submenu::new("Example Models", Menu::new()
        .add_item(g2a)
        .add_item(g2b)
    ...
    let sbml_export_parametrized = CustomMenuItem::new("sbml_export_parametrized", ".SBML (parametrized)");
    let sbml_export_instantiated = CustomMenuItem::new("sbml_export_instantiated", ".SBML (instantiated)");

    // Model submenu items
    let import_submenu = Submenu::new("Import", Menu::new()
        .add_item(local_storage)
        .add_item(aeon_import)
        .add_item(sbml_import)
        .add_submenu(examples_submenu));
    let export_submenu = Submenu::new("Export", Menu::new()
        .add_item(aeon_export)
        .add_item(sbml_export_parametrized)
        .add_item(sbml_export_instantiated));

    let model_submenu = Submenu::new("Model", Menu::new()
